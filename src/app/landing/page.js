...
const parseBookingDate = (dateString, timeString) => {
  ...
  // If date is in format "YYYY-MM-DD" or similar
...
const debugBookingData = (bookingsResponse, carId) => {
  console.log(`🔍 DEBUG: Looking for bookings for car ${carId}`);
  ...
  allBookings.forEach(booking => {
    console.log(`Booking ${booking.id}:`, {
      ...
      vehicle_object: booking.vehicle,
      pick_up_Date: booking.pick_up_Date,
      pick_up_time: booking.pick_up_time,
      drop_of_Date: booking.drop_of_Date,
      drop_of_time: booking.drop_of_time
    });
  });
  
  // Specifically look for bookings that should match car 73
  const potentialMatches = allBookings.filter(booking => 
    booking.vehicle == carId || 
    booking.vehicle_id == carId ||
    (booking.vehicle && booking.vehicle.id == carId)
  );
  
  console.log(`Potential matches for car ${carId}:`, potentialMatches);
};

const handleSearch = async () => {
  try {
    console.log("📖 Fetching all bookings with pagination...");
    ...
    do {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/booking/booking/?page=${page}`
      );
      
      const bookingsData = response.data;
      
      if (bookingsData.data && bookingsData.data.length > 0) {
        allBookings = [...allBookings, ...bookingsData.data];
        console.log(`📄 Page ${page}: ${bookingsData.data.length} bookings`);
        
        // Update total pages from the first response
        if (page === 1) {
          totalPages = bookingsData.total_pages;
          console.log(`📊 Total pages: ${totalPages}, Total records: ${bookingsData.total_records}`);
        }
      }
    ...
    // Debug: Show all bookings to verify we have the car 73 booking
    console.log("🔍 All bookings summary:");
    allBookings.forEach(booking => {
      if (booking.vehicle === 73 || (typeof booking.vehicle === 'object' && booking.vehicle.id === 73)) {
        console.log(`🚗 FOUND Car 73 Booking:`, {
          id: booking.id,
          vehicle: booking.vehicle,
          pick_up_Date: booking.pick_up_Date,
          drop_of_Date: booking.drop_of_Date,
    ...
    console.log("Total approved cars:", approvedCars.length);
    ...
    const availableCars = approvedCars.filter(car => {
      ...
      const carBookings = allBookings.filter(booking => {
        // Handle both number and object formats for vehicle
        const vehicleId = typeof booking.vehicle === 'object' ? booking.vehicle.id : booking.vehicle;
        return vehicleId == car.id; // Use loose equality
      });
      
      console.log(`Car ${car.id} (${car.name}): ${carBookings.length} bookings found`);
      
      // Debug: Show bookings for car 73
      if (car.id === 73 && carBookings.length > 0) {
        console.log(`🔍 DEBUG Car 73 bookings:`, carBookings.map(b => ({
          id: b.id,
          pickup: b.pick_up_Date,
          dropoff: b.drop_of_Date,
          vehicle: b.vehicle
        })));
      }
      ...
      if (carBookings.length === 0) {
        console.log(`✅ Car ${car.id}: No bookings - AVAILABLE`);
        return true;
      }
      
      const searchStart = pickupDate.getTime();
      const searchEnd = returnDate.getTime();

      const hasOverlappingBooking = carBookings.some(booking => {
        try {
          // Parse booking dates
          let bookingStart, bookingEnd;
          
          if (booking.pick_up_Date && booking.pick_up_time) {
            const pickupDateTime = `${booking.pick_up_Date}T${booking.pick_up_time}`;
            const dropoffDateTime = `${booking.drop_of_Date}T${booking.drop_of_time}`;
            
            bookingStart = new Date(pickupDateTime).getTime();
            bookingEnd = new Date(dropoffDateTime).getTime();
          } else {
            bookingStart = new Date(booking.pick_up_Date).getTime();
            bookingEnd = new Date(booking.drop_of_Date).getTime();
          }

          if (isNaN(bookingStart) || isNaN(bookingEnd)) {
            console.log(`⚠️ Invalid dates for booking ${booking.id}`);
            return false;
          }

          // Check for overlap
          const overlaps = searchStart < bookingEnd && searchEnd > bookingStart;
          
          if (overlaps && car.id === 73) {
            console.log(`🚫 Car 73 OVERLAP DETECTED with booking ${booking.id}:`);
            console.log(`   Search: ${new Date(searchStart).toLocaleString()} to ${new Date(searchEnd).toLocaleString()}`);
            console.log(`   Booking: ${new Date(bookingStart).toLocaleString()} to ${new Date(bookingEnd).toLocaleString()}`);
      ...
      if (hasOverlappingBooking) {
        console.log(`🚫 Car ${car.id} is BOOKED during selected period - EXCLUDING`);
      } else {
        console.log(`✅ Car ${car.id} is AVAILABLE for selected period - INCLUDING`);
      }
    ...
    console.log("\n🎯 FINAL RESULTS:");
    console.log(`Available: ${availableCars.length}/${approvedCars.length} cars`);
    availableCars.forEach(car => console.log(`- Car ${car.id}: ${car.name}`));
              ...
              <div className="bg-white rounded-xl rounded-t-none shadow-md overflow-hidden">
                <div className="p-2 px-4 border-b bg-gray-50">
                  <h2 className="text-md font-semibold text-gray-800">
                    {searchResults.length} available vehicles found
                  </h2>
                  <p className="text-xs text-gray-600">
                    Search period: {formatDate(pickupDate)} to {formatDate(returnDate)}
                  </p>
                </div>
